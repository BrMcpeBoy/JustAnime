          ...
          {[
            ...
            return (
              <Link
                to={`/az-list/${linkPath}`}
                key={index}
                className={`text-md bg-[#0a0a0a] border border-white/10 hover:border-white/20 py-1 px-4 rounded-md font-bold text-white hover:text-white hover:bg-[#1a1a1a] hover:cursor-pointer transition-all duration-300 ease-out ${
                  isActive ? "text-white bg-[#1a1a1a] border-white/20" : ""
                }`}
              >
                {item}
              </Link>
        ...
        <div>
          {categoryInfo && categoryInfo.length > 0 && (
            <CategoryCard
              ...
              showViewMore={false}
              className="mt-8"
              cardStyle="grid-cols-8 max-[1600px]:grid-cols-6 max-[1200px]:grid-cols-4 max-[758px]:grid-cols-3 max-[478px]:grid-cols-2 max-[478px]:gap-x-2"
            />
          )}
