...
class WatchTogetherSocket {
  ...
  connect(roomId, username, realUserId = null, isHost = false, animeId = null, episodeId = null, serverType = 'sub', roomType = 'public', animeTitle = null, episodeNumber = null, animePoster = null) {
    this.roomId = roomId;
    this.username = username;
    this.isHost = isHost;
    // Use real userId if provided, otherwise generate random one
    this.userId = realUserId || Math.random().toString(36).substring(7);
  ...
  // Send episode change
  sendEpisodeChange(episodeId, episodeNumber) {
    if (!this.socket || !this.connected || !this.isHost) {
      return;
    }
