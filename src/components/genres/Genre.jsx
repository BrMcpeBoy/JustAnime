    ...
    <div className="relative pt-[20px] max-sm:pt-[15px]">
      <div className="relative flex items-center min-h-[32px] max-sm:min-h-[28px]">
          ...
          <div className="flex gap-2 h-8 max-sm:h-7 items-center min-w-max px-24 max-sm:px-16">
            {data && data.map((item, index) => (
              <Link
                to={`/genre/${item}`}
                key={index}
                className="px-3.5 max-sm:px-3 h-8 max-sm:h-7 flex items-center bg-[#0a0a0a] hover:bg-[#0a0a0a] rounded-[4px] transition-all duration-300 ease-in-out group"
              >
        ...
        <div className="relative z-20 flex items-center">
          <button 
            onClick={() => scroll('left')}
            className="bg-[#0a0a0a] hover:bg-[#0a0a0a] h-8 max-sm:h-7 w-8 max-sm:w-7 flex items-center justify-center rounded-[4px] transition-all duration-300 ease-in-out focus:outline-none active:scale-95"
          >
