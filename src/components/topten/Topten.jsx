  ...
  return (
    <div className={`flex flex-col space-y-4 ${className}`}>
      <div className="flex justify-between items-center max-[350px]:flex-col max-[350px]:gap-y-2 max-[350px]:items-start">
        <h1 className="font-bold text-2xl text-white tracking-tight">{getTranslation(language, 'topTen')}</h1>
        <ul className="flex justify-between w-fit bg-[#0a0a0a] rounded-lg border border-white/10 hover:border-white/20 transition-colors overflow-hidden shadow-lg">
          {["today", "week", "month"].map((period) => (
            ...
            >
              {getTranslation(language, period)}
            </li>
          ))}
        </ul>
      </div>
