...
function Cart({ label, data, path }) {
  ...
  return (
    <div className="flex flex-col w-1/4 space-y-7 max-[1200px]:w-full">
      <h1 className="font-bold text-2xl text-[#ffffff] max-md:text-xl">
        {label}
      </h1>
      <div className="w-full space-y-4 flex flex-col">
          ...
          data.slice(0, 5).map((item, index) => (
            ...
            >
              ...
              <div className="flex flex-col ml-4 space-y-2 w-full">
                <Link
                  to={`/${item.id}`}
                  className="w-full line-clamp-2 text-[1em] font-[500] hover:cursor-pointer hover:text-[#ffffff] transform transition-all ease-out max-[1200px]:text-[14px]"
                >
        ...
        >
          <p className="text-white text-[17px] h-fit leading-4 group-hover:text-[#ffffff] transform transition-all ease-out">
            View more
          </p>
          <FaChevronRight className="text-white text-[10px] group-hover:text-[#ffffff] transform transition-all ease-out" />
        </Link>
      </div>
    </div>
  );
